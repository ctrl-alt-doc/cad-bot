...
test('page lookup translates an HTTP failure into a CAD error', async () => {
    ...
    await assert.rejects(() => new CadClient('http://localhost:5173').getPage('missing'), { message: 'CAD_HTTP_404' });
});
test('invalid JSON is translated into CAD_INVALID_RESPONSE', async () => {
    globalThis.fetch = async () => new Response('not JSON', { status: 200 });
    await assert.rejects(() => new CadClient('http://localhost:5173').listPages('reference'), { message: 'CAD_INVALID_RESPONSE' });
});
//# sourceMappingURL=cad-client.test.js.map